...
miceFast was compared[^Enviroment] with the mice package. For grouping option there was used a basic R looping and the popular dplyr package.
...
- multiple imputatiotions is faster around **x(a number of multiple imputatition)** because the core of a model is evaluated only ones.
...
[^Enviroment]: Environment: MRO 3.4.1 Intel MKL - i7 6700HQ and 24GB DDR4 2133. MRO (Microsoft R Open) provide to R a sophisticated library for linear algebra operations so remember about that when reading a performance comparision. 
...
data_DT[,bmi_imp:=impute(x=as.matrix(.SD),s="lm_bayes",2,5)] %>%
  .[,hyp_imp:=impute(x=as.matrix(.SD),s="lda",3,c(1,7)),] %>% # 7 is a new variable - bmi_imp
  .[,chl_imp:=impute_N(x=as.matrix(.SD),s="lm_noise",4,c(1,7,8),10),]
...
data = cbind(as.matrix(airquality[,-5]),
             ...
             weights = rgamma(nrow(airquality),3,3), # a numeric vector - positive values 
             index=1:nrow(airquality))

data_DT = data.table(data)

data_DT[,Ozone_imp:=impute(as.matrix(.SD),"lm_pred",1,c(6),w=.SD[['weights']]),by=.(groups)] %>%
  .[,Solar.R_imp:=impute_N(as.matrix(.SD),s="lm_bayes",2,c(3,4,5,6,10),w=.SD[['weights']],times=10),by=.(groups)]
```
...
`model`: character - a posibble options ("lda","lm_pred","lm_bayes","lm_noise")  
