...
library(miceFast);library(data.table);library(magrittr);library(mice);library(car);library(dplyr)
```
...
- where a grouping variable have to be used **(around x10 depending on data dimensions and number of groups and even more than x100 although compared to data.table only a few times faster or even the same)** because of pre-sorting by grouping variable
...
[^Environment]: Environment: MRO 3.5.3 Intel MKL - i7 6700HQ and 24GB DDR4 2133. MRO (Microsoft R Open) provide to R a sophisticated library for linear algebra operations so remember about that when reading a performance comparison. 
...
**Works with dependent variable as factor/character**:

```{r,echo=TRUE}
data_DT = data.table(mice::nhanes)

data_DT$hyp = as.factor(data_DT$hyp)

data_DT[,{Species_imp:=fill_NA(x=.SD,
                      model="lda",
                      posit_y="hyp",
                      posit_x=c('age', 'bmi', 'chl' ))}]

data_DT[,{Species_imp2:=fill_NA(x=.SD,
                         model="lda",
                         posit_y=3,
                         posit_x=c(1,2,4))}]
...
data_DT$hyp = as.character(data_DT$hyp)
...
data_DT[,Ozone_imp:=fill_NA(x=.SD, 
  ...
  .[,Solar_R_imp:=fill_NA_N(.SD,
                           model="lm_bayes",
                           posit_y='Solar.R',
                           posit_x=c('Wind','Temp','Day','Intercept','Ozone_imp'),w=.SD[['weights']],times=10),by=.(groups)]
