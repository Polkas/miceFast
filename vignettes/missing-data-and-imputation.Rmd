...
3. **Choose imputation models:** `lm_bayes` for continuous, `lda` with random `ridge`
   for categorical, `logreg = TRUE` for right-skewed variables.
...
**Important caveat:** miceFast fills variables in a single pass — each variable's
imputation conditions on the previously imputed columns but there is no iterative
cycling back (unlike mice's FCS algorithm, which iterates until convergence).
For datasets where the missing-data pattern is monotone or nearly so, a single pass
is often sufficient. With complex interlocking patterns, the single-pass approach
may introduce some bias because later variables are imputed conditional on
already-imputed (not yet converged) earlier variables. Consider using mice for
datasets with heavy non-monotone missingness.
