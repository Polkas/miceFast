...
test_that("VIF correct", {
  ...
  all_vifs_sd <- apply(
    do.call(rbind, list(vif_R[[1]], vif_R[[2]], as.vector(vif_det_miceFast))),
    2,
    sd
  )
...
testthat::test_that("VIF error", {
  expect_error(
    VIF(airquality2, 1, NULL, correct = 2),
    "is.logical\\(correct\\) is not TRUE"
  )
  expect_error(
    VIF(airquality2, 1, NULL, correct = FALSE),
    "at least two independent variables should be provided"
  )
  expect_error(
    VIF(airquality2, 1, 1, correct = FALSE),
  ...
  expect_error(
    VIF(2, 22, 2:3, correct = FALSE),
    "wrong data type - it should be data.frame, matrix or data.table"
  )
...
test_that("VIF matrix", {
  expect_true(all(
    VIF(airquality_mat, 1, c(2:5), correct = FALSE) >=
      VIF(
        airquality_mat,
        "Ozone",
...
test_that("VIF data.frame", {
  expect_true(all(
    VIF(airquality2, 1, c(2:5), correct = FALSE) >=
      VIF(
        airquality2,
        "Ozone",
        c("Solar.R", "Wind", "Temp", "Month"),
...
test_that("VIF data.table", {
  ...
  expect_true(any(
    VIF(air_miss, "Ozone", c("Solar.R", "Wind", "Temp", "Day")) >= 1
  ))
  expect_true(all(
