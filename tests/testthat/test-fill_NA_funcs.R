...
testthat::test_that("fill_NA lm_bayes accuracy - matrix", {
  ...
  testthat::expect_true(
    cor(cbind(pred_lm_bayes[index_NA], data_con[index_NA, 1]))[1, 2] > 0.5
  )
})
...
testthat::test_that("fill_NA_N lm_bayes accuracy", {
  ...
  testthat::expect_true(
    cor(cbind(pred_lm_bayes_N[index_NA], data_con[index_NA, 1]))[1, 2] > 0.5
  )
})
...
testthat::test_that("fill_NA lm_noise accuracy", {
  ...
  testthat::expect_true(
    cor(cbind(pred_lm_noise[index_NA], data_con[index_NA, 1]))[1, 2] > 0.5
  )
})
...
testthat::test_that("fill_NA_N lm_noise accuracy", {
  ...
  testthat::expect_true(
    cor(cbind(pred_lm_noise_N[index_NA], data_con[index_NA, 1]))[1, 2] > 0.5
  )
})
...
testthat::test_that("fill_NA lda accuracy", {
  ...
  testthat::expect_true(
    cor(cbind(pred_lda[index_NA], data_bin[index_NA, 1]))[1, 2] > 0.5
  )
})
...
test_that("VIF", {
  ...
  collin_x <- function(y, x) {
    ...
    vifs <- sapply(1:ncol(XXinv), function(i) {
      det(XXinv[i, i, drop = FALSE]) *
        det(XXinv[-i, -i, drop = FALSE]) /
        det(XXinv)
    })
    vifs2 <- sapply(1:ncol(XXinv), function(i) {
...
test_that("dplyr pipe pipeline fill_NA_N, fill_NA accuracy - con", {
  ...
  testthat::expect_true(all(
    cor(data_df2[
      index_NA,
      c("y_true", colnames(data_df2)[grep("y_imp", colnames(data_df2))])
    ])[, 1] >
      0.3
  ))
})
...
test_that("data.table pipeline fill_NA_N, fill_NA accuracy - con", {
  ...
  testthat::expect_true(all(
    cor(data_DT[
      index_NA,
      c("y_true", colnames(data_DT)[grep("y_imp", colnames(data_DT))]),
      with = FALSE
    ])[, 1] >
      0.3
  ))
...
testthat::test_that("dplyr pipe pipeline fill_NA_N, fill_NA - dis", {
  ...
  dd <- data_df2[
    index_NA,
    colnames(data_df2)[grep("y_imp", colnames(data_df2))]
  ]
...
testthat::test_that("data.table pipeline fill_NA/fill_NA_N accuracy - disc", {
  ...
  testthat::expect_true(all(
    data_DT[
      ...
      .[, lapply(.SD, function(x) 100 * mean(y_true == x))] >
      10
  ))
})
