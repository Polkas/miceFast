...
public:
  corrData(int _n_row, arma::vec _mu, arma::mat _cors)
      : n_row(_n_row), n_col(_mu.n_elem), mu(_mu), cors(_cors) {}

  corrData(int _nr_cat, int _n_row, arma::vec _mu, arma::mat _cors)
      : nr_cat(_nr_cat), n_row(_n_row), n_col(_mu.n_elem), mu(_mu),
        cors(_cors) {}

  arma::mat fill(std::string type);
};

std::map<std::string, int> types = {{"binom", 1}, {"discrete", 2},
                                    {"contin", 3}};
...
arma::mat corrData::fill(std::string type) {
  ...
  default: { Rcpp::stop("binom,discrete or contin"); }
  }

  for (int i = 1; i < n_col; i++) {
    X_new.col(i) = X_new.col(i) + mu(i);
  }
